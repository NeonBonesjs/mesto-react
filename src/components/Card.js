      ...
      <div className="element">
            ...
            <div className="element__name-like">
              ...
              <div className="element__like-plus-number">
                <button className={`element__like-button ${this.props.card.likes.some(i => i._id === this.context._id) ? 'element__like-button_active' : ''}`} type="button" onClick={this.handleLikeClick}/>
                <p className="element__number-like">{this.props.card.likes.length}</p>
              </div>
            </div>
            <button type="button" className={`element__trash ${!this.isOwn ? 'element__trash_invisible' : ''}`} onClick={this.handleDeleteClick}/>
          </div>
    )
  }
}
